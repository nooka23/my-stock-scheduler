...
export default function StockChartPortfolio({ data = [], trades = [], colors: {
  ...
  useEffect(() => {
    ...
    rsSeries.createPriceLine({ price: 0, color: '#9ca3af', lineWidth: 1, lineStyle: LineStyle.Dashed, axisLabelVisible: false, title: '' });
    ...
    const macdSeries = showMacd
      ? chart.addSeries(LineSeries, { color: '#2962FF', lineWidth: 1, priceScaleId: 'macd-scale', lastValueVisible: false, priceLineVisible: false, crosshairMarkerVisible: false })
      : null;

    const signalSeries = showMacd
      ? chart.addSeries(LineSeries, { color: '#FF6D00', lineWidth: 1, priceScaleId: 'macd-scale', lastValueVisible: false, priceLineVisible: false, crosshairMarkerVisible: false })
      : null;
    ...
    if (data.length > 0) {
      ...
      if (showMacd && macdSeries && signalSeries) {
    ...
    if (showLegend) { // Conditionally subscribe crosshair move
      ...
      chart.subscribeCrosshairMove((param: MouseEventParams) => {
        ...
        if (
          ...
          return;
                ...
                const macdData: any = showMacd && macdSeries ? dataMap.get(macdSeries) : null;



                const signalData: any = showMacd && signalSeries ? dataMap.get(signalSeries) : null;
                ...
                const fmtRS = (val: any) => (typeof val === 'number' && !isNaN(val) ? val.toFixed(2) : '-'); 
  ...
  return (
    ...
    <div className="relative w-full h-full">
      ...
      {showLegend && ( // Conditionally render
