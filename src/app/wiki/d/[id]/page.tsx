...
import { headingsFromContent } from '@/lib/wiki/headings';
...
export default function WikiDocumentPage() {
  ...
  return <WikiFrame><div className="wiki-shell"><main className="wiki-main"><article className="wiki-article">{error && <p className="wiki-empty">{error}</p>}{document && <>
    ...
    {document.categories.length > 0 && <div className="wiki-categories"><strong>분류:</strong>{document.categories.map((category) => <Link href={`/wiki/categories?name=${encodeURIComponent(category)}`} key={category}>{category}</Link>)}</div>}
    {headings.length > 0 && <details open className="wiki-toc"><summary>목차</summary>{headings.map((heading) => <a key={heading.id} data-level={heading.level} style={{ '--wiki-toc-depth': heading.depth } as CSSProperties} href={`#${heading.id}`}><span className="wiki-toc-number">{heading.number}</span><span>{heading.text}</span></a>)}</details>}
