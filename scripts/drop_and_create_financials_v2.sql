-- 기존 테이블 삭제 후 재생성 (quarter 컬럼 포함)

-- 1. 기존 테이블 삭제
DROP TABLE IF EXISTS company_financials_v2 CASCADE;

-- 2. 트리거 함수도 삭제 (있다면)
DROP FUNCTION IF EXISTS update_financials_v2_updated_at() CASCADE;
...
-- 인덱스 생성
CREATE INDEX idx_financials_v2_company_code ON company_financials_v2(company_code);
CREATE INDEX idx_financials_v2_year ON company_financials_v2(year);
CREATE INDEX idx_financials_v2_data_source ON company_financials_v2(data_source);
CREATE INDEX idx_financials_v2_company_year ON company_financials_v2(company_code, year);
